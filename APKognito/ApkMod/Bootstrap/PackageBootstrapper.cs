...
namespace APKognito.ApkMod.Bootstrap;
...
public sealed class PackageBootstrapper
{
    ...
    private void RemoveLauncherIntents(XmlNode node)
    {
        ...
        // XmlDocumentFragment viewFilter = _manifest.CreateDocumentFragment();
        // viewFilter.InnerXml = "<action xmlns:android=\"http://schemas.android.com/apk/res/android\" android:name=\"android.intent.action.VIEW\" />";
        // _ = node["intent-filter"]!.PrependChild(viewFilter);
    }
    ...
    {
        return Encoding.Default.GetString(BootstrapResources.BootstrapActivity);
    }

    private static string GetBootstrapperClass()
    {
        return Encoding.Default.GetString(BootstrapResources.Bootstrap);
    }
}
